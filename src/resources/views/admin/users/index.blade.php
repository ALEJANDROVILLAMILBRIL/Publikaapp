                            ...
                            <form method="GET" action="{{ route('admin.users.index') }}">
                                <div class="p-4">
                                    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-6 gap-3 items-end">
                                        ...
                                        <!-- Campo Role -->
                                        <div class="flex flex-col w-full">
                                            <label for="role_id" class="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1">{{ __('Role') }}</label>
                                            <input type="text" id="role_name" name="role_name" value="{{ request('role_name') }}"
                                    ...
                                    <tr class="border-t border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <td class="px-6 py-4">
                                            <div class="w-8 h-8 bg-blue-100 dark:bg-blue-900 rounded-full flex items-center justify-center">
                                                <span class="text-xs font-semibold text-blue-600 dark:text-blue-300">{{ $user->id }}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $user->role->name }}</div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                <i class="fas fa-calendar-alt text-gray-400 text-xs"></i>
                                                <span class="text-sm text-gray-500 dark:text-gray-400">{{ $user->created_at->format('Y-m-d') }}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <div class="flex justify-center space-x-2">
                                                ...
                                                <form id="deleteForm_{{ $user->slug }}" action="{{ route('admin.users.destroy', $user->slug) }}" method="POST" style="display: inline;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button"
                            ...
                            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-4 rounded-lg shadow mb-3">
                                ...
                                <div class="space-y-1 text-sm text-gray-500 dark:text-gray-400">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span>{{ __('Created At') }}: {{ $user->created_at->format('Y-m-d') }}</span>
                                    </div>
                                </div>
                            </div>
                        @empty
